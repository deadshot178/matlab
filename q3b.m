...
% Generate 18 Chebyshev nodes
n = 18;
x_nodes = 2.5 + 2.5 * cos((2*(1:n)-1)*pi/(2*n));
y_nodes = f(x_nodes);

% Compute the Lagrange Interpolating Polynomial p(x) using the Chebyshev nodes
p = @(x) 0;
for i = 1:n
    L = 1;
    for j = 1:n
...
plot(x_nodes, y_nodes, 'go', 'MarkerSize', 8); % Plot the Chebyshev nodes
plot(x_max_error, f(x_max_error), 'mx', 'MarkerSize', 10, 'LineWidth', 2); % Indicate the x value where the max error occurs
legend('f(x)', 'p(x)', 'Chebyshev Nodes', 'Max Error Location');
xlabel('x');
ylabel('y');
title('f(x) vs. p(x) with 18 Chebyshev nodes');
grid on;
hold off;
